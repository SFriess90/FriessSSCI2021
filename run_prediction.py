...
#for s in ['2']:
    ...
    def runExperiment():
        ...
        popmins2, pops2, _, _, _ = opt.runCMAES(objective, centroids[2], config=[10, 10, pred_sigma], gen=10 - 3, cmat=covs[2])
        c = time.time()
        print((c - b) / 60)

        print("Finished experiment.")
        
        return str([popmins, popmins[:3] + popmins2[:]])
    ...
    if __name__ == '__main__':
        ...
        out_experiments = open('experimental_data/experimental_data-predict-10-'+str(s)+'-sigma.out', 'wb')
